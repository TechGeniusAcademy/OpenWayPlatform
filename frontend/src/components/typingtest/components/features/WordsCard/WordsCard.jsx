...
import { RECITE_MODE_TITLE, RESTART_BUTTON_TOOLTIP_TITLE_WORDSCARD, SELECT_ONE_OR_MORE_CHAPTERS, SELECTIVE_MODE, VOCAB_MODE } from "../../../constants/Constants";
...
const WordsCard = ({ soundType, soundMode }) => {
  ...
  const word = (alphabetSet) => {
    if (Array.from(alphabetSet).length === 0) return "";
    const counts = [3, 4, 5];
      ...
      showCatalog && (
        <div className="Catalog">
          {Object.keys(DICTIONARY_SOURCE_CATALOG[vocabSource]).map((char) => (
            <div key={char} className={alphabetSet.has(char) ? "Catalog-li-Activated" : "Catalog-li"} onClick={() => updateAlphabetSet(char)}>
              {char.toUpperCase()}
            </div>
          ))}
        </div>
    ...
    return (
      <div className="Catalog-Selected">
        {Object.keys(DICTIONARY_SOURCE_CATALOG).map((optionSource) => (
          <div key={optionSource} className={vocabSource === optionSource ? "Catalog-li-Activated" : "Catalog-li"} onClick={() => updateVocabSource(optionSource)}>
            {optionSource}
          </div>
        ))}
      </div>
  ...
  const catalogForSource = DICTIONARY_SOURCE_CATALOG[vocabSource] || {};
  const availableChapters = Object.keys(catalogForSource).sort();

  let safeChapter = currChapter;

  // ищем следующую существующую главу в алфавите
  const nextChapter = availableChapters.find((letter) => letter > safeChapter);

  // если текущая глава недоступна, выбираем следующую, если она есть
  if (!catalogForSource[safeChapter] && nextChapter) {
    safeChapter = nextChapter;
    setCurrChapter(safeChapter);
    setIndex(0);
  }

  const chapterData = catalogForSource[safeChapter] || [0, 0];

  const currChapterStartIndex = chapterData[0];
  const currChapterEndIndex = chapterData[1];
  ...
  const getSelected = () => {
    return (
      <div className="Catalog-Selected">
        <div>{alphabetSet.size === 0 ? "Please select keys to practice from selected keys." : "Practice from keys: " + [...alphabetSet].sort().join(", ").toUpperCase()}</div>
      </div>
    );
  };
  ...
  return (
    <div className="words-card-container">
      <div className="words-card-catalog">
        ...
        <div className={!showCatalog ? "Catalog-Button" : "Catalog-Button-Activated"} onClick={toggleCatalog}>
      ...
      <div className="words-card-main">
        ...
        <div className="wordcard-word-display-field">
          ...
          {extra.map((char, idx) => (
            <span key={"wordextra" + idx} className={getExtraCharClassName(char)}>
              {char}
            </span>
          ))}
        </div>
        <div className="wordscard-UI">
          ...
          {mode === "selective" && <p>Selected Keys:</p>}
            ...
            <Grid container justifyContent="center" alignItems="center">
              ...
              <Box display="flex" flexDirection="row">
                {[...alphabetSet].sort().map((chapter) => (
                  ...
                  >
                    <span className={getChapterClassName(chapter, currChapter)}>{chapter.toUpperCase()}</span>
                  </IconButton>
                ))}
              </Box>
              {mode === "vocab" && (
      ...
      >
        <DialogTitle>
          <div>
            ...
            <span className="key-type">Tab</span> <span className="key-note">/</span> <span className="key-type">Enter</span> <span className="key-note">to restart the chapter</span>
          </div>
          <span className="key-note"> press </span>
