...
/* ── Note sub-components (right side) ── */
const NoteR1 = () => (
  <g className="note">
    <g>
...
export default function BongoCat() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.add(animatePaw(".paw-left .up"),    "start")
        .add(animatePaw(".paw-right .down"), "start")
        .add(animatePaw(".paw-left .down"),  "start+=0.19")
        .add(animatePaw(".paw-right .up"),   "start+=0.19")
        .timeScale(1.6);

      /* ── Terminal code draw-on ── */
      const termLines = el.querySelectorAll(".terminal-code line");
      const numLines  = termLines.length;
      termLines.forEach((line, i) => {
        ...
        gsap.fromTo(
          ...
          { attr: { "stroke-dashoffset": len } },
          {
            ...
            ease: "none",
            repeat: -1,
            repeatDelay: (numLines - i - 1) * 0.1,
          }
        );
      });

      /* ── Floating music notes ── */
      const allNotes = gsap.utils.shuffle([...el.querySelectorAll(".music .note")]);
      gsap.set(allNotes, { scale: 0, autoAlpha: 1 });

      const n       = Math.floor(allNotes.length / 3);
      const groups  = [allNotes.slice(0, n), allNotes.slice(n, n * 2), allNotes.slice(n * 2)];
      const colors  = ["#a5ea9b","#ff61d8","#569cfa","#ffcc81","#7ed1e2","#a3a4ec","#67b5c0","#fd7c6e"];
      const rndColor = () => colors[Math.floor(Math.random() * colors.length)];
      const rndRot   = () => Math.round(Math.random() * 100 - 50);
      const dir      = (a) => `${Math.random() > 0.5 ? "+" : "-"}=${a}`;
      ...
      const animateNotes = (notes) => {
        ...
        gsap.fromTo(
          ...
          { autoAlpha: 1, y: 0, scale: 0 },
          {
            ...
            rotation: dir(Math.round(Math.random() * 10 + 20)),
            x: dir(Math.round(Math.random() * 20 + 40)),
            y: -(Math.round(Math.random() * 20 + 200)),
            onComplete: () => animateNotes(notes),
          }
        );
      };

      tl.add(() => animateNotes(groups[0]))
        .add(() => animateNotes(groups[1]), ">0.05")
        .add(() => animateNotes(groups[2]), ">0.25");
    }, el);
  ...
  return (
    <div ref={containerRef} className="bongo-container">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 783.55 354.91">
        <g id="bongo-cat">
          ...
          <g className="music music-right">
            <NoteR1 /><NoteR1 /><NoteR1 />
            <NoteR2 /><NoteR2 /><NoteR2 />
            <NoteR3 /><NoteR3 /><NoteR3 />
            <NoteR4 /><NoteR4 /><NoteR4 />
          </g>
          ...
          <g className="music music-left">
            <NoteL1 /><NoteL1 /><NoteL1 />
            <NoteL2 /><NoteL2 /><NoteL2 />
            <NoteL3 /><NoteL3 /><NoteL3 />
            <NoteL4 /><NoteL4 /><NoteL4 />
          </g>
