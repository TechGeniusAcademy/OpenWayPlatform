...
4. Что выведет console.log(typeof "hello")?
  ...
  return (
    <div className="bulk-test-editor">
      <div className="bulk-editor-header">
        <h3>Массовое создание теста</h3>
        <button className="bulk-btn-close" onClick={onClose}>✕</button>
      </div>
      ...
      <div className="bulk-editor-content">
        {!showPreview ? (
          <>
            {/* Настройки теста */}
            <div className="bulk-test-settings">
            ...
            <div className="bulk-format-instruction">
              <h4>Формат ввода вопросов</h4>
              <div className="bulk-instruction-content">
...
2. Следующий вопрос?
   ...
   b) Правильный ответ [✓]`}</pre>
                ...
                <button className="bulk-btn-secondary" onClick={loadExample}>
                  Загрузить пример
                </button>
              </div>
            </div>
            ...
            <div className="bulk-actions">
              <button className="bulk-btn-primary" onClick={handlePreview}>
          ...
          <>
            ...
            <div className="bulk-preview-section">
              ...
              <div className="bulk-preview-questions">
                {previewQuestions.map((question, index) => (
                  <div key={index} className="bulk-preview-question">
                    <h5>Вопрос {index + 1}</h5>
                    <p className="bulk-question-text">{question.question_text}</p>
                    <div className="bulk-preview-options">
                      {question.options.map((option, optIndex) => (
                        <div key={optIndex} className={`bulk-preview-option ${option.is_correct ? 'bulk-correct' : ''}`}>
                          <span className="bulk-option-letter">{String.fromCharCode(97 + optIndex)})</span>
                          <span className="bulk-option-text">{option.option_text}</span>
                          {option.is_correct && <span className="bulk-correct-mark">✓</span>}
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="bulk-preview-actions">
                <button className="bulk-btn-success" onClick={handleImport}>
                  Создать тест
                </button>
                <button className="bulk-btn-secondary" onClick={() => setShowPreview(false)}>
