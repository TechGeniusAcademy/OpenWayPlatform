...
class User {
  ...
  static async findById(id) {
    try {
      const result = await pool.query(
        'SELECT id, username, email, role, full_name, group_id, points, experience, avatar_url, avatar_frame, profile_banner, username_style, message_color, is_group_leader, created_at FROM users WHERE id = $1',
        [id]
      );
      return result.rows[0];
    } catch (error) {
