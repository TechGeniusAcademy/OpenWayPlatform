...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($db->usernameExists($username)) {
        header("Location: register.php?error=Логин занят");
        exit;
    }

    if ($db->emailExists($username)) {
        header("Location: register.php?error=Email занят");
        exit;
    }
}
?>
...
<body>
        ...
        <div class="form-container">
            <div class="form basic">
                <form class="form" method="POST">
                    ...
                    <label><i class="fa-solid fa-envelope"></i><input type="email" placeholder="Email" name="email" required></label>
                    <label><i class="fa-solid fa-lock"></i><input type="password" placeholder="Введите пароль" name="password" required></label>
                    <label><i class="fa-solid fa-lock"></i><input type="password" placeholder="Подтвердите пароль" name="confirm_password" required></label>
                    <?php
                    if (isset($_GET["error"])):
                    ?>
    ...
    <script>
        <?php if ($db->register($username, $email, $password)): ?>
            const btn = document.getElementById("btn");
            btn.addEventListener("click", () => {
                window.location.assign("login.php?succes=еееееее");
            })
        <?php endif; ?>
    </script>
</body>
